...
enum TextAlign { LEFT, CENTER, RIGHT }; // Top, Middle, Bottom to be added later
...
private:
  ...
  void writeScrolledText(i2cDisplay *display, const char *text, int scrollPos,
                         int maxChars);
  bool checkAndUpdateLcdText(lcdText *sText);
  bool UpdateTextLines(i2cDisplay *display);
...
#define PROG_MODE_BLINK_DELAY 500 // Blink delay for "Prog Mode active" text
  ...
  ulong _showProgrammingMode_last_Blink =
      0; // Last time the blink state was updated
  bool _showProgrammingMode_showProgMode =
      true; // Toggle between showing/hiding "Prog Mode active"
...
public:
  ...
  void SetFourLines(const char *header, const char *line1, const char *line2,
                    const char *line3);
  inline void SetFourHeader(const char *header) {
    strcpy(TextHeader->text, header);
  }
  inline void SetFourLine1(const char *line1) {
    strcpy(TextLine1->text, line1);
  }
  inline void SetFourLine2(const char *line2) {
    strcpy(TextLine2->text, line2);
  }
  inline void SetFourLine3(const char *line3) {
    strcpy(TextLine3->text, line3);
  }
